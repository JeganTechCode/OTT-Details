...
function App() {
  ...
  const [overallMovieslist, setOverallMovieslist] = useState([]);

  useEffect(() => {
    const getTheSpecificSearchResult = async () => {
      try {
        console.log(window.location.href.split('/')[4]);
        const gotIMDBID = window.location.href.split('/')[4];
        const postTheimdbid = await axios.post(`${configuration.BACKEND_LOCALHOST_SERVER}${configuration.BACKEND_SPECIFIC_IMDBID}`, { gotIMDBID: gotIMDBID });
        const movieGenres = postTheimdbid.data.genre;
        setMoviesWithGenres(movieGenres); // Set the state 
        var imageurl = postTheimdbid.data.imageurl[0];
        setMovieImageUrl(imageurl);
        var released = postTheimdbid.data.released;
        setMovieReleased(released);
        var title = postTheimdbid.data.title;
        setMovieTitle(title);
        var synopsis = postTheimdbid.data.synopsis;
  ...
  useEffect(() => {
    const getTheAdvancedSearchResult = async () => {
      try {
        ...
        setOverallMovieslist(moviesWithGenres);
      } catch (error) {
        if (error.response) {
              ...
              <div class="movie-detail-content">
                ...
                <div class="meta-wrapper">
                  ...
                  <div class="ganre-wrapper">
                    {moviesWithGenres.map((genre, index) => (
                  ...
                  <div class="date-time">
                    ...
                    <div>
                      ...
                      <time datetime="2021">{movieReleased}</time>
                    </div>
          ...
          <section class="tv-series">
            <div class="container">
              ...
              <ul class="movies-list">
                ...
                {overallMovieslist.map((movie, index) => (
                  <>
                    <li key={index}>
